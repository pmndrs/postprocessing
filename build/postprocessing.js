/**
 * postprocessing v6.3.2 build Mon Apr 29 2019
...
(function (global, factory) {
  ...
  var fragment$1 = "uniform sampler2D inputBuffer;varying vec2 vUv;varying vec2 vUv0;varying vec2 vUv1;varying vec2 vUv2;varying vec2 vUv3;varying vec2 vUv4;varying vec2 vUv5;void main(){const vec2 threshold=vec2(EDGE_THRESHOLD);vec4 delta;vec3 c=texture2D(inputBuffer,vUv).rgb;vec3 cLeft=texture2D(inputBuffer,vUv0).rgb;vec3 t=abs(c-cLeft);delta.x=max(max(t.r,t.g),t.b);vec3 cTop=texture2D(inputBuffer,vUv1).rgb;t=abs(c-cTop);delta.y=max(max(t.r,t.g),t.b);vec2 edges=step(threshold,delta.xy);if(dot(edges,vec2(1.0))==0.0){discard;}vec3 cRight=texture2D(inputBuffer,vUv2).rgb;t=abs(c-cRight);delta.z=max(max(t.r,t.g),t.b);vec3 cBottom=texture2D(inputBuffer,vUv3).rgb;t=abs(c-cBottom);delta.w=max(max(t.r,t.g),t.b);float maxDelta=max(max(max(delta.x,delta.y),delta.z),delta.w);vec3 cLeftLeft=texture2D(inputBuffer,vUv4).rgb;t=abs(c-cLeftLeft);delta.z=max(max(t.r,t.g),t.b);vec3 cTopTop=texture2D(inputBuffer,vUv5).rgb;t=abs(c-cTopTop);delta.w=max(max(t.r,t.g),t.b);maxDelta=max(max(maxDelta,delta.z),delta.w);edges*=step(0.5*maxDelta,delta.xy);gl_FragColor=vec4(edges,0.0,0.0);}";
  var vertex$1 = "uniform vec2 texelSize;varying vec2 vUv;varying vec2 vUv0;varying vec2 vUv1;varying vec2 vUv2;varying vec2 vUv3;varying vec2 vUv4;varying vec2 vUv5;void main(){vUv=uv;vUv0=uv+texelSize*vec2(-1.0,0.0);vUv1=uv+texelSize*vec2(0.0,1.0);vUv2=uv+texelSize*vec2(1.0,0.0);vUv3=uv+texelSize*vec2(0.0,-1.0);vUv4=uv+texelSize*vec2(-2.0,0.0);vUv5=uv+texelSize*vec2(0.0,2.0);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}";
  ...
  var vertex$2 = "uniform vec2 texelSize;uniform vec2 halfTexelSize;uniform float kernel;/*Packing multiple texture coordinates into one varying and using a swizzle toextract them in the fragment shader still causes a dependent texture read.*/varying vec2 vUv0;varying vec2 vUv1;varying vec2 vUv2;varying vec2 vUv3;void main(){vec2 dUv=(texelSize*vec2(kernel))+halfTexelSize;vUv0=vec2(uv.x-dUv.x,uv.y+dUv.y);vUv1=vec2(uv.x+dUv.x,uv.y+dUv.y);vUv2=vec2(uv.x+dUv.x,uv.y-dUv.y);vUv3=vec2(uv.x-dUv.x,uv.y-dUv.y);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}";
  ...
  var fragment$3 = "uniform sampler2D inputBuffer;uniform float opacity;varying vec2 vUv;void main(){vec4 texel=texture2D(inputBuffer,vUv);gl_FragColor=opacity*texel;}";
  ...
  var fragment$5 = "uniform sampler2D depthBuffer0;uniform sampler2D depthBuffer1;uniform sampler2D inputBuffer;varying vec2 vUv;void main(){float d0=texture2D(depthBuffer0,vUv).r;float d1=texture2D(depthBuffer1,vUv).r;if(d0<d1){discard;}gl_FragColor=texture2D(inputBuffer,vUv);}";
  ...
  var vertexTemplate = "uniform vec2 resolution;uniform vec2 texelSize;uniform float cameraNear;uniform float cameraFar;uniform float aspect;uniform float time;varying vec2 vUv;VERTEX_HEADvoid main(){vUv=uv;VERTEX_MAIN_SUPPORTgl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}";
  ...
  var fragment$9 = "#define sampleLevelZeroOffset(t, coord, offset) texture2D(t, coord + float(offset) * texelSize, 0.0)\nuniform sampler2D inputBuffer;uniform sampler2D areaTexture;uniform sampler2D searchTexture;uniform vec2 texelSize;varying vec2 vUv;varying vec4 vOffset[3];varying vec2 vPixCoord;vec2 round(vec2 x){return sign(x)*floor(abs(x)+0.5);}float searchLength(vec2 e,float bias,float scale){e.r=bias+e.r*scale;return 255.0*texture2D(searchTexture,e,0.0).r;}float searchXLeft(vec2 texCoord,float end){/*@PSEUDO_GATHER4This texCoord has been offset by(-0.25,-0.125)in the vertex shader tosample between edge,thus fetching four edges in a row.Sampling with different offsets in each direction allows to disambiguatewhich edges are active from the four fetched ones.*/vec2 e=vec2(0.0,1.0);for(int i=0;i<MAX_SEARCH_STEPS_INT;++i){e=texture2D(inputBuffer,texCoord,0.0).rg;texCoord-=vec2(2.0,0.0)*texelSize;if(!(texCoord.x>end&&e.g>0.8281&&e.r==0.0)){break;}}texCoord.x+=0.25*texelSize.x;texCoord.x+=texelSize.x;texCoord.x+=2.0*texelSize.x;texCoord.x-=texelSize.x*searchLength(e,0.0,0.5);return texCoord.x;}float searchXRight(vec2 texCoord,float end){vec2 e=vec2(0.0,1.0);for(int i=0;i<MAX_SEARCH_STEPS_INT;++i){e=texture2D(inputBuffer,texCoord,0.0).rg;texCoord+=vec2(2.0,0.0)*texelSize;if(!(texCoord.x<end&&e.g>0.8281&&e.r==0.0)){break;}}texCoord.x-=0.25*texelSize.x;texCoord.x-=texelSize.x;texCoord.x-=2.0*texelSize.x;texCoord.x+=texelSize.x*searchLength(e,0.5,0.5);return texCoord.x;}float searchYUp(vec2 texCoord,float end){vec2 e=vec2(1.0,0.0);for(int i=0;i<MAX_SEARCH_STEPS_INT;++i){e=texture2D(inputBuffer,texCoord,0.0).rg;texCoord+=vec2(0.0,2.0)*texelSize;if(!(texCoord.y>end&&e.r>0.8281&&e.g==0.0)){break;}}texCoord.y-=0.25*texelSize.y;texCoord.y-=texelSize.y;texCoord.y-=2.0*texelSize.y;texCoord.y+=texelSize.y*searchLength(e.gr,0.0,0.5);return texCoord.y;}float searchYDown(vec2 texCoord,float end){vec2 e=vec2(1.0,0.0);for(int i=0;i<MAX_SEARCH_STEPS_INT;++i){e=texture2D(inputBuffer,texCoord,0.0).rg;texCoord-=vec2(0.0,2.0)*texelSize;if(!(texCoord.y<end&&e.r>0.8281&&e.g==0.0)){break;}}texCoord.y+=0.25*texelSize.y;texCoord.y+=texelSize.y;texCoord.y+=2.0*texelSize.y;texCoord.y-=texelSize.y*searchLength(e.gr,0.5,0.5);return texCoord.y;}vec2 area(vec2 dist,float e1,float e2,float offset){vec2 texCoord=AREATEX_MAX_DISTANCE*round(4.0*vec2(e1,e2))+dist;texCoord=AREATEX_PIXEL_SIZE*texCoord+(0.5*AREATEX_PIXEL_SIZE);texCoord.y+=AREATEX_SUBTEX_SIZE*offset;return texture2D(areaTexture,texCoord,0.0).rg;}void main(){vec4 weights=vec4(0.0);vec4 subsampleIndices=vec4(0.0);vec2 e=texture2D(inputBuffer,vUv).rg;if(e.g>0.0){vec2 d;vec2 coords;coords.x=searchXLeft(vOffset[0].xy,vOffset[2].x);coords.y=vOffset[1].y;d.x=coords.x;/*Now fetch the left crossing edges,two at a time using bilinearfiltering. Sampling at-0.25(see @CROSSING_OFFSET)enables to discern whatvalue each edge has.*/float e1=texture2D(inputBuffer,coords,0.0).r;coords.x=searchXRight(vOffset[0].zw,vOffset[2].y);d.y=coords.x;/*Translate distances to pixel units for better interleave arithmetic andmemory accesses.*/d=d/texelSize.x-vPixCoord.x;vec2 sqrtD=sqrt(abs(d));coords.y-=texelSize.y;float e2=sampleLevelZeroOffset(inputBuffer,coords,ivec2(1,0)).r;weights.rg=area(sqrtD,e1,e2,subsampleIndices.y);}if(e.r>0.0){vec2 d;vec2 coords;coords.y=searchYUp(vOffset[1].xy,vOffset[2].z);coords.x=vOffset[0].x;d.x=coords.y;float e1=texture2D(inputBuffer,coords,0.0).g;coords.y=searchYDown(vOffset[1].zw,vOffset[2].w);d.y=coords.y;d=d/texelSize.y-vPixCoord.y;vec2 sqrtD=sqrt(abs(d));coords.y-=texelSize.y;float e2=sampleLevelZeroOffset(inputBuffer,coords,ivec2(0,1)).g;weights.ba=area(sqrtD,e1,e2,subsampleIndices.x);}gl_FragColor=weights;}";
  var vertex$5 = "uniform vec2 texelSize;varying vec2 vUv;varying vec4 vOffset[3];varying vec2 vPixCoord;void main(){vUv=uv;vPixCoord=uv/texelSize;vOffset[0]=uv.xyxy+texelSize.xyxy*vec4(-0.25,0.125,1.25,0.125);vOffset[1]=uv.xyxy+texelSize.xyxy*vec4(-0.125,0.25,-0.125,-1.25);vOffset[2]=vec4(vOffset[0].xz,vOffset[1].yw)+vec4(-2.0,2.0,-2.0,2.0)*texelSize.xxyy*MAX_SEARCH_STEPS_FLOAT;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}";
  ...
  var fragment$d = "void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){float sum=inputColor.r+inputColor.g+inputColor.b;outputColor=vec4(vec3(sum/3.0),inputColor.a);}";
  ...
  var fragment$e = "uniform float factor;void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){outputColor=vec4(floor(inputColor.rgb*factor+0.5)/factor,inputColor.a);}";
  ...
  var ColorDepthEffect = function (_Effect5) {
    ...
    function ColorDepthEffect() {
      ...
      _this17 = _possibleConstructorReturn(this, _getPrototypeOf(ColorDepthEffect).call(this, "ColorDepthEffect", fragment$e, {
        blendFunction: blendFunction,
  ...
  var fragment$f = "varying vec2 vUvR;varying vec2 vUvB;void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){vec4 color=inputColor;color.r=texture2D(inputBuffer,vUvR).r;color.b=texture2D(inputBuffer,vUvB).b;outputColor=color;}";
  var vertex$6 = "uniform vec2 offset;varying vec2 vUvR;varying vec2 vUvB;void mainSupport(){vUvR=uv+offset;vUvB=uv-offset;}";
  ...
  var ChromaticAberrationEffect = function (_Effect6) {
    ...
    function ChromaticAberrationEffect() {
      ...
      return _possibleConstructorReturn(this, _getPrototypeOf(ChromaticAberrationEffect).call(this, "ChromaticAberrationEffect", fragment$f, {
        blendFunction: blendFunction,
  ...
  var fragment$g = "void mainImage(const in vec4 inputColor,const in vec2 uv,const in float depth,out vec4 outputColor){\n#ifdef INVERTED\nvec3 color=vec3(1.0-depth);\n#else\nvec3 color=vec3(depth);\n#endif\noutputColor=vec4(color,inputColor.a);}";
  ...
  var DepthEffect = function (_Effect7) {
    ...
    function DepthEffect() {
      ...
      _this18 = _possibleConstructorReturn(this, _getPrototypeOf(DepthEffect).call(this, "DepthEffect", fragment$g, {
        blendFunction: blendFunction,
  ...
  var fragment$h = "uniform vec2 angle;uniform float scale;float pattern(const in vec2 uv){vec2 point=scale*vec2(dot(angle.yx,vec2(uv.x,-uv.y)),dot(angle,uv));return(sin(point.x)*sin(point.y))*4.0;}void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){vec3 color=vec3(inputColor.rgb*10.0-5.0+pattern(uv*resolution));outputColor=vec4(color,inputColor.a);}";
  ...
  var DotScreenEffect = function (_Effect8) {
    ...
    function DotScreenEffect() {
      ...
      _this19 = _possibleConstructorReturn(this, _getPrototypeOf(DotScreenEffect).call(this, "DotScreenEffect", fragment$h, {
        blendFunction: blendFunction,
  ...
  var fragment$i = "uniform float gamma;void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){outputColor=LinearToGamma(max(inputColor,0.0),gamma);}";
  ...
  var GammaCorrectionEffect = function (_Effect9) {
    ...
    function GammaCorrectionEffect() {
      ...
      return _possibleConstructorReturn(this, _getPrototypeOf(GammaCorrectionEffect).call(this, "GammaCorrectionEffect", fragment$i, {
        blendFunction: blendFunction,
  ...
  var GlitchEffect = function (_Effect10) {
    ...
    function GlitchEffect() {
      ...
      _this20 = _possibleConstructorReturn(this, _getPrototypeOf(GlitchEffect).call(this, "GlitchEffect", fragment$j, {
        blendFunction: blendFunction,
  ...
  var GodRaysEffect = function (_Effect11) {
    ...
    function GodRaysEffect(camera, lightSource) {
      ...
      _this21 = _possibleConstructorReturn(this, _getPrototypeOf(GodRaysEffect).call(this, "GodRaysEffect", fragment$a, {
        blendFunction: blendFunction,
        attributes: EffectAttribute.DEPTH,
        uniforms: new Map([["texture", new three.Uniform(null)]])
      }));
  ...
  var fragment$k = "uniform vec2 scale;uniform float lineWidth;void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){float grid=0.5-max(abs(mod(uv.x*scale.x,1.0)-0.5),abs(mod(uv.y*scale.y,1.0)-0.5));outputColor=vec4(vec3(smoothstep(0.0,lineWidth,grid)),inputColor.a);}";
  ...
  var GridEffect = function (_Effect12) {
    ...
    function GridEffect() {
      ...
      _this22 = _possibleConstructorReturn(this, _getPrototypeOf(GridEffect).call(this, "GridEffect", fragment$k, {
        blendFunction: blendFunction,
  ...
  var HueSaturationEffect = function (_Effect13) {
    ...
    function HueSaturationEffect() {
      ...
      _this23 = _possibleConstructorReturn(this, _getPrototypeOf(HueSaturationEffect).call(this, "HueSaturationEffect", fragment$l, {
        blendFunction: blendFunction,
  ...
  var fragment$m = "void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){vec3 noise=vec3(rand(uv*time));\n#ifdef PREMULTIPLY\noutputColor=vec4(inputColor.rgb*noise,inputColor.a);\n#else\noutputColor=vec4(noise,inputColor.a);\n#endif\n}";
  ...
  var NoiseEffect = function (_Effect14) {
    ...
    function NoiseEffect() {
      ...
      _this24 = _possibleConstructorReturn(this, _getPrototypeOf(NoiseEffect).call(this, "NoiseEffect", fragment$m, {
        blendFunction: blendFunction
      }));
  ...
  var fragment$n = "uniform sampler2D edgeTexture;uniform sampler2D maskTexture;uniform vec3 visibleEdgeColor;uniform vec3 hiddenEdgeColor;uniform float pulse;uniform float edgeStrength;\n#ifdef USE_PATTERN\nuniform sampler2D patternTexture;varying vec2 vUvPattern;\n#endif\nvoid mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){vec2 edge=texture2D(edgeTexture,uv).rg;vec2 mask=texture2D(maskTexture,uv).rg;\n#ifndef X_RAY\nedge.y=0.0;\n#endif\nedge*=(edgeStrength*mask.x*pulse);vec3 color=edge.x*visibleEdgeColor+edge.y*hiddenEdgeColor;float visibilityFactor=0.0;\n#ifdef USE_PATTERN\nvec4 patternColor=texture2D(patternTexture,vUvPattern);\n#ifdef X_RAY\nfloat hiddenFactor=0.5;\n#else\nfloat hiddenFactor=0.0;\n#endif\nvisibilityFactor=(1.0-mask.y>0.0)? 1.0 : hiddenFactor;visibilityFactor*=(1.0-mask.x)*patternColor.a;color+=visibilityFactor*patternColor.rgb;\n#endif\nfloat alpha=max(max(edge.x,edge.y),visibilityFactor);\n#ifdef ALPHA\noutputColor=vec4(color,alpha);\n#else\noutputColor=vec4(color,max(alpha,inputColor.a));\n#endif\n}";
  var vertex$7 = "uniform float patternScale;varying vec2 vUvPattern;void mainSupport(){vUvPattern=uv*vec2(aspect,1.0)*patternScale;}";
  ...
  var OutlineEffect = function (_Effect15) {
    ...
    function OutlineEffect(scene, camera) {
      ...
      _this25 = _possibleConstructorReturn(this, _getPrototypeOf(OutlineEffect).call(this, "OutlineEffect", fragment$n, {
  ...
  var fragment$o = "uniform bool active;uniform vec2 d;void mainUv(inout vec2 uv){if(active){uv=vec2(d.x*(floor(uv.x/d.x)+0.5),d.y*(floor(uv.y/d.y)+0.5));}}";
  ...
  var PixelationEffect = function (_Effect16) {
    ...
    function PixelationEffect() {
      ...
      _this26 = _possibleConstructorReturn(this, _getPrototypeOf(PixelationEffect).call(this, "PixelationEffect", fragment$o, {
  ...
  var RealisticBokehEffect = function (_Effect17) {
    ...
    function RealisticBokehEffect() {
      ...
      _this27 = _possibleConstructorReturn(this, _getPrototypeOf(RealisticBokehEffect).call(this, "RealisticBokehEffect", fragment$p, {
        blendFunction: blendFunction,
        attributes: EffectAttribute.CONVOLUTION | EffectAttribute.DEPTH,
  ...
  var fragment$q = "uniform float count;void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){vec2 sl=vec2(sin(uv.y*count),cos(uv.y*count));vec3 scanlines=vec3(sl.x,sl.y,sl.x);outputColor=vec4(scanlines,inputColor.a);}";
  ...
  var ScanlineEffect = function (_Effect18) {
    ...
    function ScanlineEffect() {
      ...
      _this28 = _possibleConstructorReturn(this, _getPrototypeOf(ScanlineEffect).call(this, "ScanlineEffect", fragment$q, {
        blendFunction: blendFunction,
  ...
  var fragment$r = "uniform bool active;uniform vec2 center;uniform float waveSize;uniform float radius;uniform float maxRadius;uniform float amplitude;varying float vSize;void mainUv(inout vec2 uv){if(active){vec2 aspectCorrection=vec2(aspect,1.0);vec2 difference=uv*aspectCorrection-center*aspectCorrection;float distance=sqrt(dot(difference,difference))*vSize;if(distance>radius){if(distance<radius+waveSize){float angle=(distance-radius)*PI2/waveSize;float cosSin=(1.0-cos(angle))*0.5;float extent=maxRadius+waveSize;float decay=max(extent-distance*distance,0.0)/extent;uv-=((cosSin*amplitude*difference)/distance)*decay;}}}}";
  var vertex$8 = "uniform float size;uniform float cameraDistance;varying float vSize;void mainSupport(){vSize=(0.1*cameraDistance)/size;}";
  ...
  var ShockWaveEffect = function (_Effect19) {
    ...
    function ShockWaveEffect(camera) {
      ...
      _this29 = _possibleConstructorReturn(this, _getPrototypeOf(ShockWaveEffect).call(this, "ShockWaveEffect", fragment$r, {
  ...
  var SepiaEffect = function (_Effect20) {
    ...
    function SepiaEffect() {
      ...
      return _possibleConstructorReturn(this, _getPrototypeOf(SepiaEffect).call(this, "SepiaEffect", fragment$s, {
        blendFunction: blendFunction,
  ...
  var SMAAEffect = function (_Effect21) {
    ...
    function SMAAEffect(searchImage, areaImage) {
      ...
      _this30 = _possibleConstructorReturn(this, _getPrototypeOf(SMAAEffect).call(this, "SMAAEffect", fragment$t, {
  ...
  var SSAOEffect = function (_Effect22) {
    ...
    function SSAOEffect(camera, normalBuffer) {
      ...
      var _ref24 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
          ...
          distanceThreshold = _ref24$distanceThresh === void 0 ? 0.65 : _ref24$distanceThresh,
          _ref24$distanceFallof = _ref24.distanceFalloff,
          distanceFalloff = _ref24$distanceFallof === void 0 ? 0.1 : _ref24$distanceFallof,
          _ref24$rangeThreshold = _ref24.rangeThreshold,
          rangeThreshold = _ref24$rangeThreshold === void 0 ? 0.0015 : _ref24$rangeThreshold,
          _ref24$rangeFalloff = _ref24.rangeFalloff,
          rangeFalloff = _ref24$rangeFalloff === void 0 ? 0.01 : _ref24$rangeFalloff,
          ...
          bias = _ref24$bias === void 0 ? 0.5 : _ref24$bias;

      _classCallCheck(this, SSAOEffect);

      _this31 = _possibleConstructorReturn(this, _getPrototypeOf(SSAOEffect).call(this, "SSAOEffect", fragment$u, {
        blendFunction: blendFunction,
        attributes: EffectAttribute.DEPTH,
  ...
  var vertex$a = "uniform float scale;varying vec2 vUv2;void mainSupport(){vUv2=uv*vec2(aspect,1.0)*scale;}";
  ...
  var TextureEffect = function (_Effect23) {
    ...
    function TextureEffect() {
      ...
      _this32 = _possibleConstructorReturn(this, _getPrototypeOf(TextureEffect).call(this, "TextureEffect", fragment$a, {
        blendFunction: blendFunction,
  ...
  var ToneMappingEffect = function (_Effect24) {
    ...
    function ToneMappingEffect() {
      ...
      _this33 = _possibleConstructorReturn(this, _getPrototypeOf(ToneMappingEffect).call(this, "ToneMappingEffect", fragment$v, {
        blendFunction: blendFunction,
  ...
  var fragment$w = "uniform float offset;uniform float darkness;void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){const vec2 center=vec2(0.5);vec3 color=inputColor.rgb;\n#ifdef ESKIL\nvec2 coord=(uv-center)*vec2(offset);color=mix(color,vec3(1.0-darkness),dot(coord,coord));\n#else\nfloat d=distance(uv,center);color*=smoothstep(0.8,offset*0.799,d*(darkness+offset));\n#endif\noutputColor=vec4(color,inputColor.a);}";
  ...
  var VignetteEffect = function (_Effect25) {
    ...
    function VignetteEffect() {
      ...
      _this34 = _possibleConstructorReturn(this, _getPrototypeOf(VignetteEffect).call(this, "VignetteEffect", fragment$w, {
