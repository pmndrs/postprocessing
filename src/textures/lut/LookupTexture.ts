...
import { LUTDomainBounds } from "./LUTDomainBounds.js";
...
export class LookupTexture extends Data3DTexture {
	...
	async scaleUp(size: number, transferData = true): Promise<LookupTexture> {
		...
		return new Promise<LookupTexture>((resolve, reject) => {
	...
	override clone(): this {
		...
		return LookupTexture.from(this) as this;
	...
	static from(texture: Texture): LookupTexture {
		...
		const domainDataSrc = (texture.userData ?? {}) as LUTDomainBounds;
		const domainDataDst = lut.userData as LUTDomainBounds;

		if(domainDataSrc.domainMin !== undefined && domainDataSrc.domainMax !== undefined) {
