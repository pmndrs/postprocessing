 ...
 * screen by themselves. Every effect must provide a fragment shader that
 * implements at least `void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor)`
 * or `void mainUv(const inout vec2 uv)`. If the effect relies on depth, it must
 * implement `void mainImage(const in vec4 inputColor, const in vec2 uv, const in float depth, out vec4 outputColor)`.
 * Effects may also provide a vertex shader that implements `void mainSupport()`.
 *
 ...
 * macros as usual, but should not define global variables or constants. If the
 * camera of the associated `EffectPass` is a `PerspectiveCamera`, the macro
 * `PERSPECTIVE_CAMERA` will be defined. Furthermore, the shader chunks
 * [common](https://github.com/mrdoob/three.js/blob/dev/src/renderers/shaders/ShaderChunk/common.glsl)
