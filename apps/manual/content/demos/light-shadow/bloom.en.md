...
The `BloomEffect` uses a fast `MipmapBlurPass` to blur bright scene highlights. By default, any colors that exceed the SDR range `[0.0, 1.0]` will be affected by bloom. The luminance filter can be adjusting via the `luminanceThreshold` and `luminanceSmoothing` settings. It can also be disabled entirely by setting `luminancePass.enabled` to `false`.
